...
def create_combined_annual(year_range):
    ...
    df = pd.concat([df, sr]).rename(
        columns={"Data Date": "Date", "Hour Number": "Hour"}
    )
...
def demand_imputation(df):
    ...
    empty_reg = (
        df[[regionality, "Demand (MW)"]].groupby(by=[regionality], as_index=False).sum()
    )
    ...
    df = pd.merge(
        df,
        increment.drop(columns=["count", "median", "mean", "std"]),
        how="left",
        on=[regionality],
    )
    ...
    df.loc[df["imputed_demand"] < df["Lower_Threshold"], "imputed_demand"] = df[
        "Lower_Threshold"
    ]
    df.loc[df["imputed_demand"] > df["Upper_Threshold"], "imputed_demand"] = df[
        "Upper_Threshold"
    ]
    ...
    df.sort_values(
        by=[regionality, "UTC Time at End of Hour"], inplace=True
    )  # changed from local
    ...
    df.loc[
        ((df["imputed_demand"].isna()) & (~df["shift_demand_d"].isna())),
        "Shift_Day_Flag",
    ] = 1
    df.loc[
        ((df["imputed_demand"].isna()) & (~df["shift_demand_d"].isna())),
    ...
    df.loc[
        ((df["imputed_demand"].isna()) & (~df["shift_demand_db"].isna())),
        "Shift_Day_Flag",
    ] = 1
    df.loc[
        ((df["imputed_demand"].isna()) & (~df["shift_demand_db"].isna())),
    ...
    df.loc[
        ((df["imputed_demand"].isna()) & (~df["shift_demand_da"].isna())),
        "Shift_Day_Flag",
    ] = 1
    df.loc[
        ((df["imputed_demand"].isna()) & (~df["shift_demand_da"].isna())),
