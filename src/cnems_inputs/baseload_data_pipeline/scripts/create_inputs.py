...
def EIA930_download(year, regionality, desired_columns):
    ...
    for url in urls:
        try:
            df = pd.read_csv(
                url, usecols=desired_columns, encoding="utf-8", low_memory=False
            )

            if "Demand (MW) (Adjusted)" in df.columns:
                df = df.rename(columns={"Demand (MW) (Adjusted)": "Demand (MW)"})
            df["UTC Time at End of Hour"] = pd.to_datetime(
                df["UTC Time at End of Hour"]
            )
            df["Local Time at End of Hour"] = pd.to_datetime(
                df["Local Time at End of Hour"]
            )
            ...
            print(
                f"Error downloading or processing data for year {year} from {url}: {e}"
            )
            raise
