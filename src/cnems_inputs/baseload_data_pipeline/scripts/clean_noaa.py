...
def main():
    ...
    def f_loadmerge_ba(
        ...
        try:
            ...
            #####
            df_fips_fil = (
                session_noaa.query(ba_county)
                .filter(ba_county.BA_BASR_Code == i_ba_basr)
                .all()
            )
            ...
            df_county = (
                session_noaa.query(county)
                .filter(county.FIPS_cnty.in_(l_fips_fil))
                .all()
            )
            ...
            l_files_year = [
                x for x in l_files_year if re.search(f"-{i_year}", x) is not None
            ]
            df_weather = pd.concat(
                [pd.read_csv(f"{l_dbloc}/FIPS-NOAA/{x}") for x in l_files_year]
            )
            df_weather["FIPS_cnty"] = [
                str(x).rjust(5, "0") for x in df_weather["FIPS_cnty"]
            ]
            ...
            df_weather_pop = df_weather_pop.merge(
                right=df_weather_pop.groupby(by=["year", "month", "day", "hour"])[
                    "population"
                ]
                .sum()
                .reset_index(),
            ...
            for i_var in l_var:
                try:
                    ...
                    df_weather_pop = df_weather_pop.merge(
                        df_weather_pop.groupby(["year", "month", "day", "hour"])[
                            f"{i_var}_"
                        ]
                        .sum()
                        .reset_index(),
